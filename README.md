...
Кроме этого - на них торгуются разные инструменты. На форекс это синтетические валютные пары и CFD (Контракт на разницу цен). О инструментах биржы читай ниже.
...
Финансовые инструменты могут быть базовыми, например [акции](#Акции-shares) и [облигации](#Облигации-bonds), так и производными (**деривативы**) - это [фьючерсы](#Фьючерсы-futures) и [опционы](#Опционы-options---не-путать-с-бинарными-опционами-которые-по-сути-лохотрон). Если ты ньюфаг или мусульманин - тебе не рекомендуется торговать деривативами.
...
Фьючерс - это контрактное обязательство эмитента (продавца фьючерса) поставить некий **базовый актив** в будущем (к определённому сроку). Базовым активом может быть как сырьё (нефть, кукуруза, говно), так и акции (фьючерсы на акции газпрома) и валюты (фьючерс на доллар - **Si**).

![Продажа фьючерса](091413_0032_8.jpg)
...
При покупке фьючерса вы не платите всю стоимость сразу - биржа блокорует **гарантийное обеспечение** (**ГО**) на вашем счёте. Если фьючерс поставочный, то к дате экспирации у вас на счету должно быть достаточно средств для полной оплаты. Если фьючерс расчётный, то покупателю и продавцу начисляется вариационная маржа, равная изменению рыночной цены фьючерса.

### Опционы (Options) - не путать с бинарными(!!!) опционами, которые по сути лохотрон

Контракты, дающие возможность к определённому сроку в будущем купить/продать **базовый актив** по заранее оговорённой цене (**страйк**). В отличии от фьючерса вы не выплачиваете стоимость БА сразу, а платите лишь рыночную цену за возможность. Если к дате экспирации цена базового актива изменилась так, что вам не выгодно использовать свой опцион - то он не будет реализован. К дате экспирации вам нужно иметь необходимую сумму на счету, если базовым активом является расчётный фьючерс - то эта сумма должна быть больше или равна ГО. Опционы бывают:
