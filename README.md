...
Если в этом гайде что-то не понятно - пишите в тред, чтобы кто-нибудь дополнил или исправил этот гайд.
...
Фьючерс - это контрактное обязательство эмитента поставить некий **базовый актив** в будущем (к определённому сроку). Базовым активом может быть как сырьё (нефть, кукуруза, сахар), так и акции (фьючерсы на акции газпрома) и валюты (фьючерс на доллар - **Si**)

Цена фьючерса не всегда равна цене базового актива, т.к. если рынок ждёт изменения цены **БА**, то на этих ожиданиях может расти или падать цена на фьючерс. Разница между ценами БА и фьючерса называется **контанго/бэквардация**. Чем ближе дата экспирации фьючерса, тем эта разница меньше, т.к. тем меньше вероятность что цена БА изменится. Фьюерсы бывают:

 - поставочниые - при экспирации которых на биржу поставляется реальный актив
